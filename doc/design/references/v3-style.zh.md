...
根属性是可见状态的权威来源。RTC 使用 `microphone`、`camera`、`sharing`、`calling`、`layout`；媒体／游戏 UI 同样使用 `state`、`muted`、`controls`、`theater`、`fullscreen`、`presentation`。不要再用并行布尔量和临时 class 表示同一状态。异步动作失败时同时恢复组件和根属性。
...
- 插件是轻量协调器。具有独立数据、DOM 和交互生命周期的界面实体下沉到 `ui/components`，提供 `update/data/state/element/resize/destroy` 等同层标准接口；不要在 Conversation、Controlbar 中保留复杂私有 DOM 构建过程。IM 插件固定采用 Contacts、Conversations、Conversation、Dashboard：Conversation 通过默认 `layout` 装配 Messages、Message、Composer，Dashboard 引用 Settings。集合只在确有独立组件契约时保留，旧 Dialog、Workspace、组件级 Contacts/Conversations/Transcript 不再保留。
...
App 直接以 IM UI 的主 Tab 为框架。IM 提供 `contacts` 和 `messages`，其中 Conversations 与 Conversation 共用消息页；App 只向同一 Tab 注入 `play`、`game`、`meeting`。直播和点播共享一个 Player 实例。切换到联系人／消息时，最近活动媒体移到右侧上方并使用 `popup`；切回媒体页时，媒体恢复全尺寸，Conversation 插件移动到右侧并使用 `mini`。事件只驱动 section 和 DOM 重新挂载，模块实例与领域状态持续存活。
