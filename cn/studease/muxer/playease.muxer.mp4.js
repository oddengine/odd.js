(function(playease) {
	...
	var segmentinfo = function() {
		...
		function _init() {
			...
			_this.beginPts = 0,
			_this.endPts = 0,
			_this.originalBeginDts = 0,
			_this.originalEndDts = 0,
			_this.syncPoints = [],    // SampleInfo[n], for video IDR frames only
			_this.firstSample = null, // SampleInfo
			_this.lastSample = null;  // SampleInfo
		}
		
		_this.appendSyncPoint = function(sampleinfo) {
			sampleinfo.isSyncPoint = true;
			_this.syncPoints.push(sampleinfo);
		};
	...
	muxer.mp4 = function(config) {
		...
		function _init() {
			...
			_dtsBase = 0;
			_fillSilentAfterSeek = utils.isMSIE() || utils.isIETrident() || utils.isEdge();
		...
		_this.getVideoSegment = function(track) {
			...
			_this.dispatchEvent(events.PLAYEASE_MP4_SEGMENT, {
				tp: 'video',
				data: _this.mergeBoxes(moofbox, mdatbox),
		...
		_this.getAudioSegment = function(track) {
			...
			while (samples.length) {
				...
				if (remuxSilentFrame) {
					...
					var frame = _this.generateSilentAudio(dts, silentFrameDuration);
					if (frame == null) {
						continue;
					}
		...
		_this.generateSilentAudio = function(dts, frameDuration) {
			var unit = AAC.silentFrames[_audioMeta.channelCount - 1];
			if (unit == null) {
		...
		_this.mergeBoxes = function(moof, mdat) {
