(function(playease) {
	...
	core.view = function(model) {
		...
		function _init() {
			...
			_wrapper.setAttribute('controls', model.getConfig('controls'));
		...
		function _onRenderClick(e) {
			...
			if (time <= _previousClick + 700) {
				...
				var method = model.getConfig('doubleClick');
				var value = model.getProperty(method);
				var event = events.PLAYEASE_VIEW_FULLSCREEN;
				
				if (method == 'fullpage') {
					event = events.PLAYEASE_VIEW_FULLPAGE;
				}
				
				_this.dispatchEvent(event, { exit: value });
				return;
			}
		...
		_this.resize = function(width, height) {
			setTimeout(function() {
				...
				var ct = model.getConfig('controls');
				...
				_this.dispatchEvent(events.RESIZE, { width: width, height: height + (fs || !ct ? 0 : 40) });
