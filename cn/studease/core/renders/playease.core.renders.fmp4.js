(function(playease) {
	...
	renders.fmp4 = function(layer, config) {
		...
		function _onMP4Box(e) {
			...
			case 'esds':
				...
				if (o[3] && o[3].data) {
					o = mp4.parseDescriptor(o[3].data.subarray(3));
					if (o[4] && o[4].data) {
						o = mp4.parseDescriptor(o[4].data.subarray(13));
						if (o[5] && o[5].data) {
	...
	renders.fmp4.isSupported = function(file, mode) {
		...
		if (utils.isMSIE('(8|9|10)') || utils.isIETrident() || utils.isSogou() || utils.isQQBrowser() || utils.isIOS()
				|| utils.isAndroid('[0-4]\\.\\d') || utils.isAndroid('[5-8]\\.\\d') && utils.isChrome('([1-4]?\\d|5[0-5])\\.\\d')
				|| mode == renderModes.LIVE && (protocol == 'http' || protocol == 'https') && !window.fetch) {
			return false;
		}
